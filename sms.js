...
let Chaincode = class {
   ...
   */
  async Init(stub) {
    console.log('=========== Init: Instantiated / Upgraded sms chaincode ===========');
    return shim.success();
  }
   ...
   *    "receiver":"5556",
   *    "message":"test", 
   *    "sentDate":"2018-10-22T11:52:20.182Z"
   * }
   */
  async createSms(stub, args) {
    console.log('============= START : createSms ===========');
    console.log('##### createSms arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'sms' + json['sender'] + json['receiver'] + json['sentDate'];
    json['docType'] = 'sms';

    console.log('##### createSms payload: ' + JSON.stringify(json));
   ...
   // }
    ...
    console.log('============= END : createSms ===========');
  }

  /**
   * Retrieves a specfic sms
   * 
   * @param {*} stub 
   * @param {*} args 
   */
  async querySms(stub, args) {
    console.log('============= START : querySms ===========');
    console.log('##### querySms arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'sms' + json['sender'] + json['receiver'] + json['sentDate'];
    console.log('##### querySms key: ' + key);
   ...
   * @param {*} args 
   */
  async queryAllSms(stub, args) {
    console.log('============= START : queryAllSms ===========');
    console.log('##### queryAllSms arguments: ' + JSON.stringify(args));
 
    let queryString = '{"selector": {"docType": "sms"}}';
